...
`/api/admin/access` 返回角色矩阵、Action、绑定路由、OAuth scope；后台权限说明抽屉只读展示。
...
- **成员管理 `/admin/website/members`**：账号、邮箱、固定角色、状态；新建和调整在抽屉中完成。
  新角色默认 contributor。不能删除账号；停用保留作者归属。管理员不能调整自己、所有者或其他管理员。
  所有权转移独立入口，必须校验当前密码及已启用的 MFA，原子交接唯一 owner。
- **权限说明抽屉**：成员管理默认打开角色权限，外部应用默认打开应用授权范围；两个视图可在抽屉内切换。所有登录用户可查看，按需读取 `/api/admin/access`。
