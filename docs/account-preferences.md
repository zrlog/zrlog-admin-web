...
- GET `/api/admin/user/preferences`：返回 `{overrides, defaults, effective}`，仅含本期可编辑偏好；不返回原始 user 行和 dashboard 插件运行数据。
...
个人资料页增加「个人设置」页签，控件显示有效值，未改动项继续跟随站点，提供全部恢复默认。修改时预览外观/语言，保存后清理当前会话页面缓存。后台首页现有布局设置自动保存到当前账号，未保存者仍读取原全局布局作为默认。
...
`POST /api/admin/knowledge/chat` 继续用现有 SSE 事件，配置由服务端读取当前账号设置；旧请求 options 不再决定权限。请求开始时快照范围，后续模型轮次/工具调用/最终返回重新核对账号与设置，设置变化终止当前请求。客户端不缓存知识权限。外部 MCP 契约不变。
...
`POST /api/admin/knowledge/chat` 增加 articleId（0 表示当前账号新草稿）和 includeArticleContext，服务端从该账号持久记录构造上下文。客户端 history 保留兼容校验，但不再作为已保存历史的权威来源。请求开始、模型轮次和保存前检查账号及文章访问权限。最终答案、思考文本、引用来源和稳定 messageId 在返回 done 前保存；写入失败不能报告成功。
